...
#include "stmmac_ptp-4.19-ethercat.h"
#include "stmmac-4.19-ethercat.h"
#include <linux/reset.h>
#include <linux/of_mdio.h>
#include "dwmac1000-4.19-ethercat.h"
#include "dwxgmac2-4.19-ethercat.h"
#include "hwif-4.19-ethercat.h"
...
static int stmmac_tx_clean(struct stmmac_priv *priv, int budget, u32 queue)
{
	...
	if (!priv->ecdev && unlikely(netif_tx_queue_stopped(netdev_get_tx_queue(priv->dev,
								queue))) &&
...
static int stmmac_napi_check(struct stmmac_priv *priv, u32 chan)
{
	...
	if (!priv->ecdev && needs_work && napi_schedule_prep(&ch->napi)) {
...
static netdev_tx_t stmmac_tso_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (unlikely(stmmac_tx_avail(priv, queue) <
		...
		if (!priv->ecdev && !netif_tx_queue_stopped(netdev_get_tx_queue(dev, queue))) {
	...
	if (unlikely(stmmac_tx_avail(priv, queue) <= (MAX_SKB_FRAGS + 1))) {
		if (!priv->ecdev) {
			...
			netdev_err(priv->dev, "%s: stop transmitted packets\n",
				   __func__);		
		}
	}
	...
	if (!priv->ecdev && likely(priv->tx_coal_frames > tx_q->tx_count_frames) &&
...
static int stmmac_rx(struct stmmac_priv *priv, int limit, u32 queue)
{
	...
	while (count < limit) {
		...
		if (unlikely(status == discard_frame)) {
			...
			if (priv->ecdev) {
				ecdev_receive(priv->ecdev, skb->data, frame_len);
				dev_kfree_skb_any(skb);
			} else {
		     ...
		     struct stmmac_resources *res)
{
	...
	dev_info(priv->device, "%s: ecdev: 0x%lx\n", __func__, (unsigned long)priv->ecdev);
...
int ethercat_stmmac_suspend(struct device *dev)
{
	...
	if ((!ndev || !netif_running(ndev)) && !priv->ecdev)
		return 0;
...
int ethercat_stmmac_resume(struct device *dev)
{
	...
	if (!netif_running(ndev) && !priv->ecdev)
		return 0;
